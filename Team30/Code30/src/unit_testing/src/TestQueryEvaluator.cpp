...
class QeFactoryMethods {
 public:
  static std::shared_ptr<PqlDeclaration> getDeclaration(std::string name, EntityType type) {
    return std::make_shared<PqlDeclaration>(
        std::make_shared<std::string>(name),
        type);
  }

  static std::unique_ptr<SelectClause> getSelectClause(std::string name, EntityType type) {
    return std::make_unique<SelectClause>(getDeclaration(name, type));
  }

  static std::unique_ptr<FollowsClause> getFollowsClause(StmtRef ref1, StmtRef ref2) {
...
TEST_CASE("Select and Follows Clause with 0 common declarations") {
  ...
  result = *qe.evaluateQuery(std::move(select_clause), std::move(follows_clause));
  REQUIRE(result == CONSTANTS);
  ...
  // Case where follows clause has declarations that are different from select clause
  select_clause = QeFactoryMethods::getSelectClause("a", EntityType::STMT);
  follows_clause = QeFactoryMethods::getFollowsClause(StmtRef(QeFactoryMethods::getDeclaration("b", EntityType::STMT)),
...
TEST_CASE("Select and Follows Clause with 1 common declarations") {
  ...
  select_clause = QeFactoryMethods::getSelectClause("a", EntityType::STMT);
  follows_clause = QeFactoryMethods::getFollowsClause(StmtRef(QeFactoryMethods::getDeclaration("a", EntityType::STMT)),
                                                      StmtRef(QeFactoryMethods::getDeclaration("s", EntityType::STMT)));
  result = *qe.evaluateQuery(std::move(select_clause), std::move(follows_clause));
  REQUIRE(result == std::vector<std::string>({"2"}));

  select_clause = QeFactoryMethods::getSelectClause("abc", EntityType::STMT);
  follows_clause = QeFactoryMethods::getFollowsClause(StmtRef(),
                                                      StmtRef(QeFactoryMethods::getDeclaration("abc", EntityType::STMT)));
  result = *qe.evaluateQuery(std::move(select_clause), std::move(follows_clause));
  REQUIRE(result == std::vector<std::string>({"1","2"}));

  select_clause = QeFactoryMethods::getSelectClause("abc", EntityType::STMT);
  follows_clause = QeFactoryMethods::getFollowsClause(StmtRef(1),
                                                      StmtRef(QeFactoryMethods::getDeclaration("abc", EntityType::STMT)));
  result = *qe.evaluateQuery(std::move(select_clause), std::move(follows_clause));
  REQUIRE(result == std::vector<std::string>({"2"}));

  select_clause = QeFactoryMethods::getSelectClause("abc", EntityType::STMT);
  follows_clause = QeFactoryMethods::getFollowsClause(StmtRef(QeFactoryMethods::getDeclaration("abc", EntityType::STMT)),
                                                      StmtRef(1));
  result = *qe.evaluateQuery(std::move(select_clause), std::move(follows_clause));
  ...
  select_clause = QeFactoryMethods::getSelectClause("a", EntityType::STMT);
  follows_clause = QeFactoryMethods::getFollowsClause(StmtRef(2),
