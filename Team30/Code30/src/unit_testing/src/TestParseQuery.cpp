...
/* TEST_CASE("Parse select and such that query") { */
/*   std::vector<std::shared_ptr<Token>> tokens; */
...
/*   std::shared_ptr<SelectClause> selectClause =
 * std::static_pointer_cast<SelectClause>(clauseQueue.front()); */
/*   REQUIRE(strcmp(selectClause->getDeclaration()->getName()->c_str(), "s "));
 */
/*   /1* /2* REQUIRE(*(selectClause->getDeclaration()->getName()) == "s"); *2/
 * *1/ */
/*   /1* CHECK(selectClause->getDeclaration()->getName() == "s "); *1/ */
