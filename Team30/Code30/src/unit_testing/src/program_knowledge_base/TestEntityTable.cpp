#include "../../../spa/src/program_knowledge_base/entities/EntityDatabase.h"
#include "catch.hpp"
...
TEST_CASE("Successful insert and retrieval") {
  ...
  std::unordered_set<std::string> if_set = db.Get(EntityType::IF);
  REQUIRE(if_set.find("1") != if_set.end());
  REQUIRE(if_set.find("3") == if_set.end());

  std::unordered_set<std::string> while_set = db.Get(EntityType::WHILE);
