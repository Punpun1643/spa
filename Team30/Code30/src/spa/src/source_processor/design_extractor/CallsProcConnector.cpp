...
void CallsProcConnector::connectProcsAndUpdateRelations(
  ...
  while (!procNodeMap_copy.empty()) {
    ...
    if (minCalls != 0) {
      ...
      std::cout << "ERROR: cyclic call detected\n";
    }
    ...
    usesRelations =
        *pkb.getRelationValues(procNameWMinCalls, EntityType::VARIABLE, USES_P);
