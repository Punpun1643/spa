...
void QueryEvaluator::PopulateIntermediateResultsTable(
  ...
  for (const auto& clause : clauses) {
    auto clause_result = (clause->SupportsConditionalEvaluation == 2) ? EvaluateClauseConditionally(table, clause) : clause->Evaluate(pkb);
    if (clause->IsNegated()) {
