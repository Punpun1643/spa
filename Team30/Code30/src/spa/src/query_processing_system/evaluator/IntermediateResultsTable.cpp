...
std::vector<std::vector<std::string>> IntermediateResultsTable::getValuesGivenDeclarations(
  ...
  for (auto& decl: decls) {
    if (table_mapping[decl] != table_idx) {
      mergeExistingTables(table_idx, table_mapping[decl]);
    }
  }
  ...
  auto results = relevant_table.getTableCols(decls);
  ArrayUtility::removeDuplicates(results);
  return results;
}
...
bool IntermediateResultsTable::hasDeclaration(PqlDeclaration const& declaration){
  return table_mapping.count(declaration) == 1;
}

void IntermediateResultsTable::mergeExistingTables(int table_to_keep_id, int table_to_merge_id) {
  tables[table_to_keep_id].join(tables[table_to_merge_id]);
