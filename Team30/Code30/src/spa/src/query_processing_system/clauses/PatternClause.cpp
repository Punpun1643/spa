...
PatternClause::PatternClause(PqlDeclaration decl, EntRef ent_ref, EntityType target_type)
:decl(std::move(decl)), ent_ref(std::move(ent_ref)) {
  ...
  if (this->ent_ref.GetRefType() == PqlRefType::DECLARATION &&
    ...
    throw InvalidSemanticsException(
        "If the entRef of a Pattern clause is a declaration, it must have the variable type");
  }
}
  ...
  switch (ent_ref.GetRefType()) {
    case (PqlRefType::DECLARATION): {
      ...
      return std::make_unique<ClauseResult>(
          decl, ent_ref.GetDeclaration(), values);
    }
