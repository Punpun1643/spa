...
class NotClauseDecorator: public Clause {
 private:
  ...
  static std::vector<std::string> NegateSingleDeclValues(PKBQPSInterface& pkb, PqlDeclaration const& decl, std::unordered_set<std::string> const& value_set);
  static std::vector<std::pair<std::string, std::string>> NegateDoubleDeclValues(PKBQPSInterface& pkb, PqlDeclaration const& decl_1, PqlDeclaration const& decl_2,
