...
class PriorityScorer {
  ...
  private:
    ...
    float const NUM_SYNONYMS_PRIORITY_NORMALISER = 1.0 * pow(10, -0);
    /* float const NUM_SYNONYMS_PRIORITY_NORMALISER = 0; */

    /* float const NUM_NEW_SYNONYMS_PRIORITY_NORMALISER = 1.0 * pow(10, -1); */
    float const NUM_NEW_SYNONYMS_PRIORITY_NORMALISER = 0;

    /* float const NEGATED_CLAUSE_NORMALISER = 1.0 * pow(10, -2); */
    float const NEGATED_CLAUSE_NORMALISER = 0;

    /* float const CLAUSE_TYPE_PRIORITY_NORMALISER = 1.0 * pow(10, -3); */
    float const CLAUSE_TYPE_PRIORITY_NORMALISER = 0;

    std::shared_ptr<Context> context;
    std::unordered_set<std::string> queried_synonyms;

    float CalculatePriorityScore(int const num_synonyms,
        int const num_new_synonyms, int const negated, int const clause_type_priority);
    void IncrementSynonymPriorities(std::string string, int& num_synonyms, int& num_new_synonyms);
};
